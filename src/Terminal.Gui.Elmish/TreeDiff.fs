...
module Differ =
    ...
    let toElementId (element: TerminalElement) =
        ...
        | _ ->
            match
                ...
                |> List.tryPick (fun e ->
                    match e with
                    | :? KeyValue as KeyValue("id", id) -> Some id
                    | _ -> None)
            with
            | Some id -> id.ToString()
            | None -> ""
    ...
    let (|OnlyPropsChanged|_|) (ve1: TerminalElement, ve2: TerminalElement) =
        let cve1 =
            ve1.children
            |> List.map (fun e -> e.name + "_" + (e |> toElementId))
            |> List.sort
        ...
        if ve1 |> toElementId <> (ve2 |> toElementId) || cve1 <> cve2 then
            None
        else
            Some()
    ...
    let rec update (rootTree: TerminalElement) (newTree: TerminalElement) =
        ...
        | rt, nt when rt.name <> nt.name || (rt |> toElementId <> (nt |> toElementId)) ->
            let id1 = rt |> toElementId
            let id2 = nt |> toElementId
        ...
        | ChildsDifferent ->
            ...
            let groupedRootType =
                sortedRootChildren |> List.map (fun v -> v.name) |> List.distinct

            let groupedNewType =
                sortedNewChildren |> List.map (fun v -> v.name) |> List.distinct
