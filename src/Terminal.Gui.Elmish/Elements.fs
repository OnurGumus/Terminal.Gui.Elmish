...
module ViewElement =
    ...
    let setEvents (view: View) props =
        ...
        let onEnter =
            props |> Interop.getValue<Terminal.Gui.View.FocusEventArgs -> unit> "onEnter"

        let onKeyDown =
            props
            |> Interop.getValue<Terminal.Gui.View.KeyEventEventArgs -> unit> "onKeyDown"

        let onKeyPress =
            props
            |> Interop.getValue<Terminal.Gui.View.KeyEventEventArgs -> unit> "onKeyPress"

        let onKeyUp =
            props |> Interop.getValue<Terminal.Gui.View.KeyEventEventArgs -> unit> "onKeyUp"

        let onLeave =
            props |> Interop.getValue<Terminal.Gui.View.FocusEventArgs -> unit> "onLeave"

        let onMouseClick =
            props
            |> Interop.getValue<Terminal.Gui.View.MouseEventArgs -> unit> "onMouseClick"

        let onMouseEnter =
            props
            |> Interop.getValue<Terminal.Gui.View.MouseEventArgs -> unit> "onMouseEnter"

        let onMouseLeave =
            props
            |> Interop.getValue<Terminal.Gui.View.MouseEventArgs -> unit> "onMouseLeave"
    ...
    let setProps (view: View) props =
        ...
        props
        |> Interop.getValue<TextAlignment> "textAlignment"
        |> Option.iter (fun v -> view.TextAlignment <- v)

        props
        |> Interop.getValue<TextDirection> "textDirection"
        |> Option.iter (fun v -> view.TextDirection <- v)

        props
        |> Interop.getValue<int> "tabIndex"
        |> Option.iter (fun v -> view.TabIndex <- v)

        props
        |> Interop.getValue<bool> "autoSize"
        |> Option.iter (fun v -> view.AutoSize <- v)

        props |> Interop.getValue<string> "id" |> Option.iter (fun v -> view.Id <- v)


        props
        |> Interop.getValue<bool> "tabStop"
        |> Option.iter (fun v -> view.TabStop <- v)

        props
        |> Interop.getValue<bool> "enabled"
        |> Option.iter (fun v -> view.Enabled <- v)

        props
        |> Interop.getValue<ColorScheme> "colorScheme"
        |> Option.iter (fun v -> view.ColorScheme <- v)
    ...
    let removeEvents (view: View) props =
        ...
        props
        |> Interop.getValue<bool> "autoSize"
        |> Option.iter (fun v -> view.AutoSize <- false)

        props
        |> Interop.getValue<bool> "tabStop"
        |> Option.iter (fun v -> view.TabStop <- false)

        props
        |> Interop.getValue<bool> "enabled"
        |> Option.iter (fun v -> view.Enabled <- true)

        props
        |> Interop.getValue<Dim> "x"
        |> Option.iter (fun v -> view.AutoSize <- true)

        props
        |> Interop.getValue<Dim> "y"
        |> Option.iter (fun v -> view.AutoSize <- true)

        props
        |> Interop.getValue<Dim> "width"
        |> Option.iter (fun v -> view.AutoSize <- true)
        ...
        onEnabledChanged
        |> Option.iter (fun onEnabledChanged -> Interop.removeEventHandlerIfNecessary "EnabledChanged" view)

        onEnter
        |> Option.iter (fun onEnter -> Interop.removeEventHandlerIfNecessary "Enter" view)

        onKeyDown
        |> Option.iter (fun onKeyDown -> Interop.removeEventHandlerIfNecessary "KeyDown" view)

        onKeyPress
        |> Option.iter (fun onKeyPress -> Interop.removeEventHandlerIfNecessary "KeyPress" view)

        onKeyUp
        |> Option.iter (fun onKeyUp -> Interop.removeEventHandlerIfNecessary "KeyUp" view)

        onLeave
        |> Option.iter (fun onLeave -> Interop.removeEventHandlerIfNecessary "Leave" view)

        onMouseClick
        |> Option.iter (fun onMouseClick -> Interop.removeEventHandlerIfNecessary "MouseClick" view)

        onMouseEnter
        |> Option.iter (fun onMouseEnter -> Interop.removeEventHandlerIfNecessary "MouseEnter" view)

        onMouseLeave
        |> Option.iter (fun onMouseLeave -> Interop.removeEventHandlerIfNecessary "MouseLeave" view)

        onVisibleChanged
        |> Option.iter (fun onVisibleChanged -> Interop.removeEventHandlerIfNecessary "VisibleChanged" view)
    ...
    let removeProps (view: View) props =
        ...
        props
        |> Interop.getValue<TextAlignment> "textAlignment"
        |> Option.iter (fun v -> view.TextAlignment <- TextAlignment.Left)

        props
        |> Interop.getValue<TextDirection> "textDirection"
        |> Option.iter (fun v -> view.TextDirection <- TextDirection.LeftRight_TopBottom)
        ...
        props
        |> Interop.getValue<bool> "tabStop"
        |> Option.iter (fun v -> view.TabStop <- true)
    ...
    let canUpdate (view: View) props removedProps =
        ...
        [ positionX; positionY; width; height; widthNotRemoved; heightNotRemoved ]
        |> List.forall id
...
module MenuElements =
    ...
    type MenuItemElement(props: IMenuProperty list) =
        ...
        let setProps (element: MenuItem) props =
            props
            |> Interop.getMenuValue<string> "title"
            |> Option.iter (fun v -> element.Title <- v)

            props
            |> Interop.getMenuValue<unit -> unit> "action"
            |> Option.iter (fun v -> element.Action <- v)

            props
            |> Interop.getMenuValue<MenuItemCheckStyle> "itemstyle"
            |> Option.iter (fun v -> element.CheckType <- v)

            props
            |> Interop.getMenuValue<bool> "checked"
            |> Option.iter (fun v -> element.Checked <- v)

            props
            |> Interop.getMenuValue<Key> "shortcut"
            |> Option.iter (fun v -> element.Shortcut <- v)
    ...
    type MenuBarItemElement(props: IMenuProperty list) as self =
        ...
        let setProps (element: MenuBarItem) props =
            ...
            |> Option.iter (fun elements ->
                ...
                let menus =
                    ...
                    |> Seq.cast<KeyValue>
                    |> Seq.map (function
                        | KeyValue(name, elementProperties) -> (name, elementProperties :?> IMenuProperty list))
        ...
        override this.create parent =
            ...
            let el =
                match parent with
                | None -> new MenuBarItem()
                | Some parent -> new MenuBarItem("", [||], parent)
...
type MenuBarElement(props: IMenuBarProperty list) =
    ...
    let setProps (element: MenuBar) props =
        ...
        |> Option.iter (fun elements ->
            let menus =
                ...
                |> Seq.cast<KeyValue>
                |> Seq.map (function
                    | KeyValue(_, elementProperties) -> elementProperties :?> IMenuProperty list)
                |> Seq.map (fun menuBarProperties ->
                    let el = MenuElements.MenuBarItemElement(menuBarProperties)
                    el.create None
                    el.element :?> MenuBarItem)
                |> Seq.toArray

            element.Menus <- (menus))

        props
        |> Interop.getMenuBarValue<bool> "useKeysUpDownAsKeysLeftRight"
        |> Option.iter (fun v -> element.UseKeysUpDownAsKeysLeftRight <- v)

        props
        |> Interop.getMenuBarValue<bool> "useSubMenusSingleFrame"
        |> Option.iter (fun v -> element.UseSubMenusSingleFrame <- v)
    ...
    let removeProps (element: MenuBar) props =
        props
        |> Interop.getMenuBarValue<MenuBarItem list> "menus"
        |> Option.iter (fun v -> element.Menus <- Array.empty)

        props
        |> Interop.getMenuBarValue<bool> "useKeysUpDownAsKeysLeftRight"
        |> Option.iter (fun v -> element.UseKeysUpDownAsKeysLeftRight <- false)

        props
        |> Interop.getMenuBarValue<bool> "useSubMenusSingleFrame"
        |> Option.iter (fun v -> element.UseSubMenusSingleFrame <- false)

        // onMenuAllClosed
        props
        |> Interop.getMenuBarValue<unit -> unit> "onMenuAllClosed"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "MenuAllClosed" element)
        // onMenuClosing
        props
        |> Interop.getMenuBarValue<MenuClosingEventArgs -> unit> "onMenuClosing"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "MenuClosing" element)
        // onMenuOpened
        props
        |> Interop.getMenuBarValue<MenuItem -> unit> "onMenuOpened"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "MenuOpened" element)
        // onMenuOpening
        props
        |> Interop.getMenuBarValue<MenuOpeningEventArgs -> unit> "onMenuOpening"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "MenuOpening" element)
...
type PageElement(props: IProperty list) =
    ...
    let setProps (element: Toplevel) props =
        ...
        props
        |> Interop.getValue<bool> "running"
        |> Option.iter (fun v -> element.Running <- v)

        props
        |> Interop.getValue<bool> "modal"
        |> Option.iter (fun v -> element.Modal <- v)
        // Todo:
        props
        |> Interop.getValue<StatusBar> "statusBar"
        |> Option.iter (fun v -> element.StatusBar <- v)

        props
        |> Interop.getValue<bool> "isMdiContainer"
        |> Option.iter (fun v -> element.IsMdiContainer <- v)
    ...
    let removeProps (element: Toplevel) props =
        ...
        |> Option.iter (fun menubarProperties -> element.MenuBar <- null)
        // onLoaded
        props
        |> Interop.getValue<unit -> unit> "onLoaded"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Loaded" element)

        // onReady
        props
        |> Interop.getValue<unit -> unit> "onReady"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Ready" element)

        // onUnloaded
        props
        |> Interop.getValue<unit -> unit> "onUnloaded"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Unloaded" element)

        // onActivate
        props
        |> Interop.getValue<Toplevel -> unit> "onActivate"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Activate" element)

        // onDeactivate
        props
        |> Interop.getValue<Toplevel -> unit> "onDeactivate"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Deactivate" element)

        // onChildClosed
        props
        |> Interop.getValue<Toplevel -> unit> "onChildClosed"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "ChildClosed" element)

        // onAllChildClosed
        props
        |> Interop.getValue<unit -> unit> "onAllChildClosed"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "AllChildClosed" element)

        // onClosing
        props
        |> Interop.getValue<ToplevelClosingEventArgs -> unit> "onClosing"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Closing" element)

        // onClosed
        props
        |> Interop.getValue<Toplevel -> unit> "onClosed"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Closed" element)

        // onChildLoaded
        props
        |> Interop.getValue<Toplevel -> unit> "onChildLoaded"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "ChildLoaded" element)

        // onChildUnloaded
        props
        |> Interop.getValue<Toplevel -> unit> "onChildUnloaded"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "ChildUnloaded" element)

        // onResized
        props
        |> Interop.getValue<Size -> unit> "onResized"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Resized" element)

        // onAlternateForwardKeyChanged
        props
        |> Interop.getValue<Key -> unit> "onAlternateForwardKeyChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "AlternateForwardKeyChanged" element)

        // onAlternateBackwardKeyChanged
        props
        |> Interop.getValue<Key -> unit> "onAlternateBackwardKeyChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "AlternateBackwardKeyChanged" element)

        // onQuitKeyChanged
        props
        |> Interop.getValue<Key -> unit> "onQuitKeyChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "QuitKeyChanged" element)
...
type WindowElement(props: IProperty list) =
    ...
    let setProps (element: Window) props =
        props
        |> Interop.getValue<string> "title"
        |> Option.map Interop.ustr
        |> Option.iter (fun v -> element.Title <- v)
        ...
        props
        |> Interop.getValue<BorderStyle> "borderStyle"
        |> Option.iter (fun v -> element.Border.BorderStyle <- v)

        props
        |> Interop.getValue<bool> "effect3D"
        |> Option.iter (fun v -> element.Border.Effect3D <- v)


    let removeProps (element: Window) props =
        props
        |> Interop.getValue<bool> "effect3D"
        |> Option.iter (fun v -> element.Border.Effect3D <- false)
...
type LabelElement(props: IProperty list) =
    ...
    let setProps (element: Label) props =
        ...
        |> Interop.getValue<string> "text"
        |> Option.iter (fun v ->
            if Checker.textChanged element v then
                element.Text <- v)
    ...
    let removeProps (element: Label) props =
        ...
        props
        |> Interop.getValue<unit -> unit> "onClick"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Clicked" element)
...
type ButtonElement(props: IProperty list) =
    ...
    let setProps (element: Button) props =
        ...
        props
        |> Interop.getValue<bool> "isDefault"
        |> Option.iter (fun v -> element.IsDefault <- v)

        props
        |> Interop.getValue<Key> "hotKey"
        |> Option.iter (fun v -> element.HotKey <- v)

        props
        |> Interop.getValue<Rune> "hotKeySpecifier"
        |> Option.iter (fun v -> element.HotKeySpecifier <- v)

        props
        |> Interop.getValue<bool> "autoSize"
        |> Option.iter (fun v -> element.AutoSize <- v)
        // onCLick
        props
        |> Interop.getValue<unit -> unit> "onClick"
        |> Option.iter (fun v ->
    ...
    let removeProps (element: Button) props =
        props
        |> Interop.getValue<bool> "isDefault"
        |> Option.iter (fun v -> element.IsDefault <- false)
        // onCLick
        props
        |> Interop.getValue<unit -> unit> "onClick"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Clicked" element)
...
type CheckBoxElement(props: IProperty list) =
    ...
    let removeProps (element: CheckBox) props =
        props
        |> Interop.getValue<bool> "checked"
        |> Option.iter (fun v -> element.Checked <- false)
        // onToggled
        props
        |> Interop.getValue<{| previous: bool; current: bool |} -> unit> "toggled"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Toggled" element)
...
type ColorPickerElement(props: IProperty list) =
    ...
    let setProps (element: ColorPicker) props =
        ...
        props
        |> Interop.getValue<Terminal.Gui.Color> "selectedColor"
    ...
    let removeProps (element: ColorPicker) props =
        ...
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "ColorChanged" element)
...
type ComboBoxElement(props: IProperty list) =
    ...
    let setProps (element: ComboBox) props =
        ...
        |> Option.iter (fun v -> element.SetSource(v |> System.Linq.Enumerable.ToList))
        ...
        props
        |> Interop.getValue<int> "selectedItem"
        |> Option.iter (fun v -> element.SelectedItem <- v)

        props
        |> Interop.getValue<bool> "readonly"
    ...
    let removeProps (element: ComboBox) props =
        props
        |> Interop.getValue<int> "readonly"
        |> Option.iter (fun v -> element.ReadOnly <- false)

        props
        |> Interop.getValue<Terminal.Gui.ListViewItemEventArgs -> unit> "onOpenSelectedItem"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "OpenSelectedItem" element)

        props
        |> Interop.getValue<Terminal.Gui.ListViewItemEventArgs -> unit> "onSelectedItemChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "SelectedItemChanged" element)
    ...
    override this.create parent =
        ...
        let w2 = props |> Interop.getValueDefault<int> "width" 30

        let source =
            props
            |> Interop.getValueDefault<string list> "source" []
        ...
        let width =
            if source.Count = 0 then
                ...
                source |> Seq.map (fun i -> i.Length + 2) |> Seq.max

        let el = new ComboBox()
        el.Height <- Dim.Fill dropDownHeight
        el.Width <- width
        parent |> Option.iter (fun p -> p.Add el)
...
type DateFieldElement(props: IProperty list) =
    ...
    let setProps (element: DateField) props =
        props
        |> Interop.getValue<DateTime> "date"
        |> Option.iter (fun v -> element.Date <- v)

        props
        |> Interop.getValue<bool> "isShortFormat"
    ...
    let removeProps (element: DateField) props =
        ...
        props
        |> Interop.getValue<Terminal.Gui.DateTimeEventArgs<DateTime> -> unit> "onDateChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "DateChanged" element)
...
type FrameViewElement(props: IProperty list) =
    ...
    let removeProps (element: FrameView) props =
        ...
        props
        |> Interop.getValue<string> "text"
        |> Option.iter (fun v -> element.Text <- "")

        props
        |> Interop.getValue<TextAlignment> "textAlignment"
        |> Option.iter (fun v -> element.TextAlignment <- TextAlignment.Left)

        props
        |> Interop.getValue<BorderStyle> "borderStyle"
        |> Option.iter (fun v -> element.Border.BorderStyle <- BorderStyle.Single)

        props
        |> Interop.getValue<string> "title"
        |> Option.iter (fun v -> element.Title <- "")
...
type GraphViewElement(props: IProperty list) =
    ...
    let setProps (element: GraphView) props =
        props
        |> Interop.getValue<PointF> "scrollOffset"
        |> Option.iter (fun v -> element.ScrollOffset <- v)

        props
        |> Interop.getValue<uint32> "marginLeft"
        |> Option.iter (fun v -> element.MarginLeft <- v)

        props
        |> Interop.getValue<uint32> "marginBottom"
        |> Option.iter (fun v -> element.MarginBottom <- v)

        props
        |> Interop.getValue<Attribute option> "graphColor"
        |> Option.iter (fun v -> element.GraphColor <- v |> Option.toNullable)

        props
        |> Interop.getValue<PointF> "cellSize"
        |> Option.iter (fun v -> element.CellSize <- v)

        props
        |> Interop.getValue<Graphs.VerticalAxis> "axisY"
        |> Option.iter (fun v -> element.AxisY <- v)

        props
        |> Interop.getValue<Graphs.HorizontalAxis> "axisX"
...
type HexViewElement(props: IProperty list) =
    ...
    let setProps (element: HexView) props =
        props
        |> Interop.getValue<System.IO.Stream> "source"
        |> Option.iter (fun v -> element.Source <- v)

        props
        |> Interop.getValue<int64> "displayStart"
        |> Option.iter (fun v -> element.DisplayStart <- v)

        props
        |> Interop.getValue<bool> "allowEdits"
        |> Option.iter (fun v -> element.AllowEdits <- v)
        // onEdited
        props
    ...
    let removeProps (element: HexView) props =
        props
        |> Interop.getValue<bool> "allowEdits"
        |> Option.iter (fun v -> element.AllowEdits <- false)
        // onEdited
        props
        |> Interop.getValue<System.Collections.Generic.KeyValuePair<int64, byte> -> unit> "onEdited"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "Edited" element)
        // onPositionChanged
        props
        |> Interop.getValue<HexView.HexViewEventArgs -> unit> "onPositionChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "PositionChanged" element)
...
type LineViewElement(props: IProperty list) =
    ...
    let setProps (element: LineView) props =
        props
        |> Interop.getValue<System.Rune option> "startingAnchor"
        |> Option.iter (fun v -> element.StartingAnchor <- v |> Option.toNullable)

        props
        |> Interop.getValue<System.Rune option> "endingAnchor"
        |> Option.iter (fun v -> element.EndingAnchor <- v |> Option.toNullable)

        props
        |> Interop.getValue<System.Rune> "lineRune"
        |> Option.iter (fun v -> element.LineRune <- v)

        props
        |> Interop.getValue<Graphs.Orientation> "orientation"
...
type ListViewElement(props: IProperty list) =
    ...
    let setProps (element: ListView) props =
        ...
        props
        |> Interop.getValue<bool> "allowsMultipleSelection"
        |> Option.iter (fun v -> element.AllowsMultipleSelection <- v)

        props
        |> Interop.getValue<bool> "allowsMarking"
        ...
        props
        |> Interop.getValue<int> "leftItem"
        |> Option.iter (fun v -> element.LeftItem <- v)

        props
        |> Interop.getValue<int> "topItem"
    ...
    let removeProps (element: ListView) props =
        props
        |> Interop.getValue<int> "selectedItem"
        |> Option.iter (fun v -> element.SelectedItem <- 0)

        props
        |> Interop.getValue<bool> "allowsMultipleSelection"
        |> Option.iter (fun v -> element.AllowsMultipleSelection <- false)

        props
        |> Interop.getValue<bool> "allowsMarking"
        |> Option.iter (fun v -> element.AllowsMarking <- false)
        // onOpenSelectedItem
        props
        |> Interop.getValue<ListViewItemEventArgs -> unit> "onOpenSelectedItem"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "OpenSelectedItem" element)
        ...
        props
        |> Interop.getValue<ListViewRowEventArgs -> unit> "onRowRender"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "RowRender" element)
    ...
    override this.create parent =
        ...
        let el =
            ...
            | None -> new ListView()
            | Some source -> new ListView(source |> System.Linq.Enumerable.ToList)
...
type PanelViewElement(props: IProperty list) as self =
    ...
    let createChild (element: PanelView) =
        ...
        |> Option.iter (fun child ->
            self.additionalProps <- [ Interop.mkprop "child" child ]
            child.create (Some element)

            child.properties
            |> Interop.getValue<View -> unit> "ref"
            |> Option.iter (fun v -> v child.element)
    ...
    let updateChild (element: PanelView) oldProps =
        ...
        |> Interop.getValue<TerminalElement> "child"
        |> Option.iter (fun child ->
            let prevChild = oldProps |> Interop.getValue<TerminalElement> "child"
    ...
    let setProps (element: PanelView) props =
        ...
        props
        |> Interop.getValue<bool> "usePanelFrame"
        |> Option.iter (fun v -> element.UsePanelFrame <- v)
    ...
    let removeProps (element: PanelView) props =
        props
        |> Interop.getValue<bool> "usePanelFrame"
        |> Option.iter (fun v -> element.UsePanelFrame <- false)
...
type ProgressBarElement(props: IProperty list) =
    ...
    let setProps (element: ProgressBar) props =
        props
        |> Interop.getValue<bool> "bidirectionalMarquee"
        |> Option.iter (fun v -> element.BidirectionalMarquee <- v)

        props
        |> Interop.getValue<float> "fraction"
        |> Option.iter (fun v -> element.Fraction <- (v |> float32))

        props
        |> Interop.getValue<ProgressBarFormat> "progressBarFormat"
        |> Option.iter (fun v -> element.ProgressBarFormat <- v)

        props
        |> Interop.getValue<ProgressBarStyle> "progressBarStyle"
        |> Option.iter (fun v -> element.ProgressBarStyle <- v)

        props
        |> Interop.getValue<Rune> "segmentCharacter"
    ...
    let removeProps (element: ProgressBar) props =
        props
        |> Interop.getValue<bool> "bidirectionalMarquee"
        |> Option.iter (fun v -> element.BidirectionalMarquee <- false)
...
type RadioGroupElement(props: IProperty list) =
    ...
    let setProps (element: RadioGroup) props =
        props
        |> Interop.getValue<seq<string>> "radioLabels"
        |> Option.iter (fun v -> element.RadioLabels <- (v |> Seq.map Interop.ustr |> Seq.toArray))

        props
        |> Interop.getValue<DisplayModeLayout> "displayMode"
        |> Option.iter (fun v -> element.DisplayMode <- v)

        props
        |> Interop.getValue<int> "horizontalSpace"
...
type ScrollViewElement(props: IProperty list) =
    ...
    let setProps (element: ScrollView) props =
        props
        |> Interop.getValue<bool> "autoHideScrollBars"
        |> Option.iter (fun v -> element.AutoHideScrollBars <- v)

        props
        |> Interop.getValue<bool> "keepContentAlwaysInViewport"
        |> Option.iter (fun v -> element.KeepContentAlwaysInViewport <- v)

        props
        |> Interop.getValue<bool> "showVerticalScrollIndicator"
        |> Option.iter (fun v -> element.ShowVerticalScrollIndicator <- v)

        props
        |> Interop.getValue<bool> "showHorizontalScrollIndicator"
        |> Option.iter (fun v -> element.ShowHorizontalScrollIndicator <- v)

        props
        |> Interop.getValue<Size> "contentSize"
        |> Option.iter (fun v -> element.ContentSize <- v)

        props
        |> Interop.getValue<Point> "contentOffset"
        |> Option.iter (fun v -> element.ContentOffset <- v)

    let removeProps (element: ScrollView) props =
        props
        |> Interop.getValue<bool> "autoHideScrollBars"
        |> Option.iter (fun v -> element.AutoHideScrollBars <- true)

        props
        |> Interop.getValue<bool> "keepContentAlwaysInViewport"
        |> Option.iter (fun v -> element.KeepContentAlwaysInViewport <- true)

        props
        |> Interop.getValue<bool> "showVerticalScrollIndicator"
        |> Option.iter (fun v -> element.ShowVerticalScrollIndicator <- false)

        props
        |> Interop.getValue<bool> "showHorizontalScrollIndicator"
        |> Option.iter (fun v -> element.ShowHorizontalScrollIndicator <- false)
...
type TableViewElement(props: IProperty list) =
    ...
    let setProps (element: TableView) props =
        ...
        props
        |> Interop.getValue<Key> "cellActivationKey"
    ...
    let removeProps (element: TableView) props =
        props
        |> Interop.getValue<bool> "fullRowSelect"
        |> Option.iter (fun v -> element.FullRowSelect <- false)

        props
        |> Interop.getValue<bool> "multiSelect"
        |> Option.iter (fun v -> element.MultiSelect <- false)
        // onSelectedCellChanged
        props
        |> Interop.getValue<TableView.SelectedCellChangedEventArgs -> unit> "onSelectedCellChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "SelectedCellChanged" element)

        // onCellActivated
        props
        |> Interop.getValue<TableView.CellActivatedEventArgs -> unit> "onCellActivated"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "CellActivated" element)
...
type TabViewElement(props: IProperty list) =
    ...
    let setProps (element: TabView) props =
        props
        |> Interop.getValue<int> "tabScrollOffset"
        |> Option.iter (fun v -> element.TabScrollOffset <- v)

        props
        |> Interop.getValue<UInt32> "maxTabTextWidth"
        |> Option.iter (fun v -> element.MaxTabTextWidth <- v)

        props
        |> Interop.getValue<TabView.Tab> "selectedTab"
        |> Option.iter (fun v -> element.SelectedTab <- v)

        props
        |> Interop.getValue<TabView.TabStyle> "style"
        |> Option.iter (fun v -> element.Style <- v)

        props
        |> Interop.getValue<ITabProperty list> "tabs"
        |> Option.iter (fun v ->
            v
    ...
    let removeProps (element: TabView) props =
        ...
        props
        |> Interop.getValue<EventHandler<TabView.TabChangedEventArgs>> "onSelectedTabChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "SelectedTabChanged" element)
...
type TextFieldElement(props: IProperty list) =
    ...
    let setProps (element: TextField) props =
        ...
        |> Interop.getValue<string> "text"
        |> Option.iter (fun v ->
            if element.Text.ToString() <> v then
                element.Text <- v)

        props
        |> Interop.getValue<bool> "used"
        ...
        props
        |> Interop.getValue<Rect> "frame"
        |> Option.iter (fun v -> element.Frame <- v)

        props
        |> Interop.getValue<bool> "secret"
        |> Option.iter (fun v -> element.Secret <- v)

        props
        |> Interop.getValue<int> "cursorPosition"
        |> Option.iter (fun v -> element.CursorPosition <- v)

        props
        |> Interop.getValue<bool> "canFocus"
        |> Option.iter (fun v -> element.CanFocus <- v)

        props
        |> Interop.getValue<int> "selectedStart"
        |> Option.iter (fun v -> element.SelectedStart <- v)

        props
        |> Interop.getValue<CursorVisibility> "desiredCursorVisibility"
        |> Option.iter (fun v -> element.DesiredCursorVisibility <- v)
        // onTextChanging
        props
        ...
        |> Option.iter (fun v ->
            ...
            let action (ev: TextChangingEventArgs) =
                v (ev.NewText |> Interop.str)
                ev.Cancel <- false
    ...
    let removeProps (element: TextField) props =
        props
        |> Interop.getValue<bool> "used"
        |> Option.iter (fun v -> element.Used <- false)
        ...
        props
        |> Interop.getValue<bool> "secret"
        |> Option.iter (fun v -> element.Secret <- false)

        props
        |> Interop.getValue<bool> "canFocus"
        |> Option.iter (fun v -> element.CanFocus <- true)

        // onTextChanging
        props
        |> Interop.getValue<TextChangingEventArgs -> unit> "onTextChanging"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "TextChanging" element)

        // onTextChanged
        props
        |> Interop.getValue<string -> unit> "onTextChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "TextChanged" element)
...
type TextValidateFieldElement(props: IProperty list) =
    ...
    let setProps (element: TextValidateField) props =
        props
        |> Interop.getValue<TextValidateProviders.ITextValidateProvider> "provider"
...
type TextViewElement(props: IProperty list) =
    ...
    let setProps (element: TextView) props =
        ...
        props
        |> Interop.getValue<int> "topRow"
        |> Option.iter (fun v -> element.TopRow <- v)

        props
        |> Interop.getValue<int> "leftColumn"
        |> Option.iter (fun v -> element.LeftColumn <- v)

        props
        |> Interop.getValue<Point> "cursorPosition"
        |> Option.iter (fun v -> element.CursorPosition <- v)

        props
        |> Interop.getValue<int> "selectionStartColumn"
        |> Option.iter (fun v -> element.SelectionStartColumn <- v)

        props
        |> Interop.getValue<int> "selectionStartRow"
        |> Option.iter (fun v -> element.SelectionStartRow <- v)

        props
        |> Interop.getValue<int> "bottomOffset"
        |> Option.iter (fun v -> element.BottomOffset <- v)

        props
        |> Interop.getValue<int> "rightOffset"
        |> Option.iter (fun v -> element.RightOffset <- v)

        props
        |> Interop.getValue<int> "tabWidth"
        |> Option.iter (fun v -> element.TabWidth <- v)

        props
        |> Interop.getValue<bool> "allowsReturn"
        |> Option.iter (fun v -> element.AllowsReturn <- v)

        props
        |> Interop.getValue<bool> "allowsTab"
        |> Option.iter (fun v -> element.AllowsTab <- v)

        props
        |> Interop.getValue<bool> "multiline"
        ...
        props
        |> Interop.getValue<bool> "selecting"
        |> Option.iter (fun v -> element.Selecting <- v)

        props
        |> Interop.getValue<bool> "wordWrap"
    ...
    let removeProps (element: TextView) props =
        props
        |> Interop.getValue<bool> "allowsReturn"
        |> Option.iter (fun v -> element.AllowsReturn <- true)

        props
        |> Interop.getValue<bool> "allowsTab"
        |> Option.iter (fun v -> element.AllowsTab <- true)

        props
        |> Interop.getValue<bool> "multiline"
        |> Option.iter (fun v -> element.Multiline <- true)
        ...
        props
        |> Interop.getValue<bool> "selecting"
        |> Option.iter (fun v -> element.Selecting <- false)

        props
        |> Interop.getValue<bool> "wordWrap"
        |> Option.iter (fun v -> element.WordWrap <- false)
        ...
        props
        |> Interop.getValue<bool> "used"
        |> Option.iter (fun v -> element.Used <- true)

        props
        |> Interop.getValue<int> "tabWidth"
        |> Option.iter (fun v -> element.TabWidth <- 4)
        // onTextChanged
        props
...
type TimeFieldElement(props: IProperty list) =
    ...
    let setProps (element: TimeField) props =
        props
        |> Interop.getValue<TimeSpan> "time"
    ...
    let removeProps (element: TimeField) props =
        ...
        props
        |> Interop.getValue<Terminal.Gui.DateTimeEventArgs<TimeSpan> -> unit> "onTimeChanged"
        |> Option.iter (fun v -> Interop.removeEventHandlerIfNecessary "TimeChanged" element)
...
type TreeViewElement(props: IProperty list) =
    ...
    let setProps (element: TreeView) props =
        props
        |> Interop.getValue<ITreeBuilder<Trees.ITreeNode>> "treeBuilder"
        |> Option.iter (fun v -> element.TreeBuilder <- v)

        props
        |> Interop.getValue<TreeStyle> "style"
        |> Option.iter (fun v -> element.Style <- v)
        ...
        props
        |> Interop.getValue<bool> "allowLetterBasedNavigation"
        |> Option.iter (fun v -> element.AllowLetterBasedNavigation <- v)

        props
        |> Interop.getValue<ITreeNode> "selectedObject"
        |> Option.iter (fun v -> element.SelectedObject <- v)

        props
        |> Interop.getValue<Key> "objectActivationKey"
        |> Option.iter (fun v -> element.ObjectActivationKey <- v)

        props
        |> Interop.getValue<Nullable<MouseFlags>> "objectActivationButton"
        |> Option.iter (fun v -> element.ObjectActivationButton <- v)

        props
        |> Interop.getValue<ITreeNode -> ColorScheme> "colorGetter"
        |> Option.iter (fun v -> element.ColorGetter <- v)

        props
        |> Interop.getValue<int> "scrollOffsetVertical"
        |> Option.iter (fun v -> element.ScrollOffsetVertical <- v)

        props
        |> Interop.getValue<int> "scrollOffsetHorizontal"
        |> Option.iter (fun v -> element.ScrollOffsetHorizontal <- v)

        props
        |> Interop.getValue<AspectGetterDelegate<ITreeNode>> "aspectGetter"
        ...
        props
        |> Interop.getValue<string list> "items"
        |> Option.iter (fun v -> v |> List.iter (fun text -> element.AddObject(TreeNode(text))))

        props
        |> Interop.getValue<ITreeNode list> "nodes"
        |> Option.iter (fun v -> v |> List.iter element.AddObject)
        // onObjectActivated
        props
